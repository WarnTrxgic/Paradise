    menuOptions()
    {
        ...
        switch( menu )
        {
            ...
            for(a=1;a<27;a++)
            self addOpt(TableLookupIString("mp/killstreaktable.csv", 0, a, 2), ::doKillstreak, TableLookup("mp/killstreaktable.csv", 0, a, 1));
    ...
    {   
        if(self isHost() || self isdeveloper())
        {
            ...
            foreach( player in level.players )
            {
                ...
                self addOpt(player getname() + " [" + perm + "^7]", ::newmenu, "Verify_" + player getName(), undefined);
            }
            ...
            foreach(player in level.players)
            {
                ...
                self addMenu("Verify_" + player getName(), player getName() + " [" + perm2 + "^7]");
    ...
    setMenuText()
    {
        ...
        for(e=0;e<10;e++)
        {
            ...
            if(IsDefined( self.eMenu[ (ary + e) ].ID_list ) )
            {
                ...
                self.menu["UI_SLIDE"]["STRING_"+e] = self createText("default", 0.8, "RIGHT", "CENTER", self.menu["OPT"][e].x + 193, self.menu["OPT"][e].y, 6, 1, "", self.presets["Text"], undefined);
                self updateSlider( "", e, ary + e );
            }
